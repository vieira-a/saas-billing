...
C4Context
  ...
  Rel(customer, billing-system, "Makes payment requests (Product subscription)")
  Rel(external-system, billing-system, "Sends webhooks with payment events")
  Rel(billing-system, redis, "Publishes payment events to the queue (Bull + Redis)")
```
